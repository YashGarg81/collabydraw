...
import { BgFill, FillStyle, FontFamily, FontSize, FontStyle, RoughStyle, StrokeEdge, StrokeFill, StrokeStyle, StrokeWidth, TextAlign, ToolType } from "@/types/canvas"
import { fillStyleIcons, fillStyleLabels, fontFamilyIcons, fontFamilyLabels, fontSizeIcons, fontSizeLabels, fontStyleIcons, fontStyleLabels, roughStyleIcons, roughStyleLabels, strokeEdgeIcons, strokeEdgeLabels, strokeStyleIcons, strokeStyleLabels, textAlignIcons, textAlignLabels } from "@/config/canvasTypeMappings";
...
export function StyleConfigurator({
    ...
    const fontStyleOptions: FontStyle[] = ['normal', 'bold', 'italic', 'bold italic']
                    ...
                    {(activeTool === "text") && (
                        <>
                            ...
                            <div className="Font-Style-Selector">
                                <ItemLabel label="Font style" />
                                <div className="flex flex-wrap gap-x-2 gap-y-2 items-center py-1">
                                    {fontStyleOptions.map((fs, index) => (
                                        <FontStyleSelector
                                            key={index}
                                            fontStyle={fontStyle}
                                            fontStyleProp={fs}
...
const FontStyleSelector = ({ fontStyle, fontStyleProp, onClick }: { fontStyle: FontStyle, fontStyleProp: FontStyle, onClick?: () => void }) => {
    return (
        <label className={cn("active flex justify-center items-center w-8 h-8 p-0 box-border border border-default-border-color rounded-lg cursor-pointer bg-light-btn-bg2 text-text-primary-color dark:bg-w-button-hover-bg dark:hover:bg-tool-btn-bg-hover-dark dark:text-text-primary-color dark:border-w-button-hover-bg focus-within:shadow-shadow-tool-focus",
            fontStyle === fontStyleProp ? 'bg-selected-tool-bg-light dark:bg-selected-tool-bg-dark dark:border-selected-tool-bg-dark' : ''
        )}
            title={fontStyleLabels[fontStyleProp]}
            onClick={onClick}
        >
            <Input type="radio" checked={fontStyle === fontStyleProp} onChange={() => onClick?.()} name="fontStyle" className="opacity-0 absolute pointer-events-none" />
            {fontStyleIcons[fontStyleProp]}
        </label>
    )
}
